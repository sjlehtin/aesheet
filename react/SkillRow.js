...
class SkillRow extends React.Component {
  ...
  render() {
      ...
      for (let ii = 0; ii < this.props.renderForStats.length; ii++) {
        ...
        checkList.push(
          <tr key={ii}>
            ...
            <td>
              <SkillCheck skillCheck={check} />
            </td>
          </tr>,
        );
      }
      checks = (
        <table aria-label={"Skill checks for explicitly given stats"}>
    ...
    if (cs && cs._missingRequired && cs._missingRequired.length > 0) {
      ...
      missing = `Missing skill${pluralize} ${cs._missingRequired.join(", ")}`;
      skillStyle.color = "red";
    }
    return (
      <tr style={skillStyle} title={missing} aria-label={"Skill"}>
        <td>
          <span style={Object.assign(skillStyle, { paddingLeft: indent })}>
            {this.props.skillName}
          </span>
          <span style={{ position: "relative" }}>{remove}</span>
        </td>
        <td>
          <span aria-label={"Skill level"}>{this.skillLevel()}</span>
        ...
        <td className="skill-check">{checks}</td>
      </tr>
    );
  }
}
